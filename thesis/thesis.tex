...
and the rest are inconclusive in both directions. The honest reading is that
this study establishes that feedback helps, does not establish which actuator
helps more, and is not powered to declare the two interchangeable.
...
therefore stimulate different skin, and the bulkier EM housing also changed how
well hand tracking performed while it was worn. A preference for one actuator
may reflect smoother control, or a more sensitive site, rather than a better
sensation.
...
cycles to ring up and ring down, whereas the EM latch transitions almost
instantly. A
...
Several descriptively large differences do not reach significance, and the
equivalence tests of Section~\ref{sec:lra-vs-em} are inconclusive for three of
four metrics. At $N=22$ the study can detect the survival effect but is
underpowered for the smaller actuator-versus-actuator comparison it was
designed around.
...
resonant actuators or electromagnetic bistable pin actuators,
...
made the grasping of fragile objects safer. Object survival rose from 62\%
under visual-only feedback to 81\% with the LRA and 78\% with the EM, a
difference that holds up statistically for both actuators, and the number of
grip adjustments made after the grasp was complete fell reliably under the EM.
Excess deformation, the force applied after the tactile display had saturated,
fell by close to an order of magnitude, although that reduction does not reach
significance at this sample size. Deformable objects, which lack a failure
threshold, showed no effect. Participants preferred both haptic conditions to
visual-only feedback and most often chose the LRA, but neither the
questionnaire nor any objective metric separated the two actuators.
...
these conditions and at this sample size, does not demonstrably matter.
Ranking actuator types would require a study that removes the confounds
identified in Section~\ref{sec:limitations}, namely counterbalanced condition
order, matched placement on the hand, resonance-tuned amplitude-modulated LRA
drive, and a force proxy calibrated in newtons.
